...
int main()
{
  ...
  if (txtEntrada == NULL)
  {
    printf("Arquivo textoEntrada.txt com erro ao abrir. Verifique se o arquivo esta na mesma pasta do codigo ou se o nome esta correto e inicie o programa novamente.\n");
    return 1;
  }
  ...
  while (fscanf(txtEntrada, "%s", palavra) != EOF) // faz a separação palavra por palavra do texto de entrada
  {
    if (strlen(palavra) > 0) // ignora possíveis caracteres nulos
    {
      ...
      if (procuraPalavra == -1)
      { // se a busca retorna -1, significa que não encontrou a palavra e devemos adicioná-la
        int i = 0;
  ...
  if (tamanhoListaPalavras > 0) // apenas adiciona no dicionario se existir palavras no texto de entrada
  {
  ...
  {
    printf("Arquivo vazio (palavras diferentes no dicionario=0). \n");
  }
